...
public class FileHelper {
    ...
    public static List<Recording> listRecordings(Context context) {
        ...
        List<Recording> fileList = new ArrayList<Recording>();
        for (DocumentFile file : files) {
            if (!file.getName().matches(Constant.FILE_NAME_PATTERN)) {
                Log.e("CallRecord", String.format("'%s' didn't match the file name pattern",
                        file.getName()));
                continue;
            }
    ...
    public static Uri getContentUri(Context context, Uri uri) {
        ...
        return FileProvider.getUriForFile(context,
                "com.kss.AudioRecordUploader.fileprovider",
